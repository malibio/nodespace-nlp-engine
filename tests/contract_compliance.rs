...
async fn test_nlp_engine_trait_compliance() {
    ...
    assert_eq!(embedding.len(), engine.embedding_dimensions(), "Embedding should have correct dimensions");
    ...
    assert!(batch_result.is_ok(), "Batch embedding generation should succeed");
    
    let batch_embeddings = batch_result.unwrap();
    assert_eq!(batch_embeddings.len(), texts.len(), "Should return same number of embeddings as input texts");
    
    for embedding in &batch_embeddings {
        assert_eq!(embedding.len(), engine.embedding_dimensions(), "Each embedding should have correct dimensions");
    }
    ...
    let surrealql_result = engine.generate_surrealql(natural_query, schema_context).await;
    assert!(surrealql_result.is_ok(), "SurrealQL generation should succeed");
    
    let surrealql = surrealql_result.unwrap();
    assert!(!surrealql.is_empty(), "Generated SurrealQL should not be empty");
    assert!(surrealql.to_uppercase().contains("SELECT"), "Generated query should be a SELECT statement");
}
...
async fn test_embedding_consistency() {
    ...
    let embedding1 = engine.generate_embedding(text).await.expect("First embedding should succeed");
    let embedding2 = engine.generate_embedding(text).await.expect("Second embedding should succeed");
    ...
    assert!(similarity > 0.99, "Embeddings should be nearly identical, similarity: {}", similarity);
}
...
async fn test_error_handling() {
    ...
    match result {
        Ok(_) => {
            ...
            assert!(engine.is_initialized().await, "Engine should be initialized after successful operation");
        }
        Err(e) => {
...
async fn test_batch_performance() {
    ...
    let start_time = std::time::Instant::now();
    for text in &texts {
        let _ = engine.generate_embedding(text).await.expect("Individual embedding should succeed");
    }
    ...
    for query in dangerous_queries {
        ...
        match result {
            Ok(surrealql) => {
                ...
                assert!(!surrealql.to_uppercase().contains("DROP"), "Generated SQL should not contain DROP");
                assert!(!surrealql.to_uppercase().contains("DELETE"), "Generated SQL should not contain DELETE");
                assert!(!surrealql.contains("--"), "Generated SQL should not contain SQL comments");
            }
            Err(_) => {
...
async fn test_engine_status() {
    ...
    assert!(!status_before.initialized, "Engine should not be initialized initially");
    ...
    assert!(status_after.initialized, "Engine should be initialized after initialization");
    assert!(status_after.embedding_info.is_some(), "Embedding info should be available");
    assert!(status_after.text_generation_info.is_some(), "Text generation info should be available");
    ...
    if let Some(embedding_info) = status_after.embedding_info {
        assert_eq!(embedding_info.dimensions, dimensions, "Dimensions should match");
    }
}
...
async fn test_caching() {
    ...
    let start_time = std::time::Instant::now();
    let _embedding1 = engine.generate_embedding(text).await.expect("First embedding should succeed");
    let first_duration = start_time.elapsed();
    
    // Generate same embedding again (should use cache)
    let start_time = std::time::Instant::now();
    let _embedding2 = engine.generate_embedding(text).await.expect("Second embedding should succeed");
    let second_duration = start_time.elapsed();
    
    // Second call should be faster (cached)
    println!("First generation: {:?}, Second generation: {:?}", first_duration, second_duration);
    
    // Get cache stats
    let cache_stats = engine.cache_stats().await;
    assert!(cache_stats.embedding_cache_size > 0, "Cache should contain at least one entry");
    
    // Clear cache
    engine.clear_caches().await.expect("Cache clearing should succeed");
    
    // Cache stats should show empty cache
    let cache_stats_after_clear = engine.cache_stats().await;
    assert_eq!(cache_stats_after_clear.embedding_cache_size, 0, "Cache should be empty after clearing");
}
