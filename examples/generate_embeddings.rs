...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, text) in texts.iter().enumerate() {
        ...
        println!("   ✓ Generated embedding with {} dimensions in {:?}", embedding.len(), duration);
    ...
    let start_time = std::time::Instant::now();
    let batch_embeddings = engine.batch_embeddings(&texts.iter().map(|s| s.to_string()).collect::<Vec<_>>()).await?;
    let batch_duration = start_time.elapsed();
    
    println!("   ✓ Generated {} embeddings in batch in {:?}", batch_embeddings.len(), batch_duration);
    ...
    for (i, (text_idx, similarity)) in similarities.iter().enumerate() {
        println!("   {}. \"{}\" (similarity: {:.3})", i + 1, texts[*text_idx], similarity);
    }
    
    // Show cache statistics
    let cache_stats = engine.cache_stats().await;
    println!("\n📈 Cache Statistics:");
    println!("   Cached embeddings: {}/{}", cache_stats.embedding_cache_size, cache_stats.embedding_cache_capacity);
